...
const JobRequirementsDisplay: React.FC<JobRequirementsDisplayProps> = ({ job, onUpdate, canEdit = false }) => {
  ...
  const muted = 'text-gray-400 italic';

  return (
    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-2xl border border-blue-100">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-sm font-semibold text-blue-800">Requisitos del Puesto</h3>
        {canEdit && (
          <div className="flex gap-2">
            {!isEditing ? (
              <button onClick={() => setIsEditing(true)} className="p-1.5 rounded-full bg-blue-100 hover:bg-blue-200 transition-colors">
                <PencilIcon className="h-4 w-4 text-blue-600" />
              </button>
            ) : (
              <>
                <button onClick={handleSave} className="p-1.5 rounded-full bg-green-100 hover:bg-green-200 transition-colors">
                  <CheckIcon className="h-4 w-4 text-green-600" />
                </button>
                <button onClick={handleCancel} className="p-1.5 rounded-full bg-red-100 hover:bg-red-200 transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isEditing ? (
              ...
              <p className={`text-sm truncate ${formData.seniority ? 'text-blue-900 font-semibold' : muted}`}>
                {formData.seniority || 'Seleccionar'}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isEditing ? (
              ...
              <p className={`text-sm truncate ${formData.englishLevel ? 'text-green-900 font-semibold' : muted}`}>
                {formData.englishLevel || 'Seleccionar'}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isEditing ? (
              ...
              <p className={`text-sm truncate ${formData.contractType ? 'text-purple-900 font-semibold' : muted}`}>
                {formData.contractType || 'Seleccionar'}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isEditing ? (
              ...
              <p className={`text-sm truncate ${formData.location ? 'text-orange-900 font-semibold' : muted}`}>
                {formData.location || 'Seleccionar'}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {isEditing ? (
              ...
              <p className={`text-sm ${job.industry_experience ? 'text-indigo-900 font-semibold' : muted}`}>
                {job.industry_experience
                  ? (job.industry_experience.required ? 'Requerida' : 'No requerida') + (job.industry_experience.required && job.industry_experience.industry ? ` · ${job.industry_experience.industry}` : '')
                  : 'Seleccionar'}
              </p>
            )}
          </div>
      ...
      <div className="mt-4 pt-4 border-t border-blue-200">
        ...
        {isEditing ? (
          ...
          <p className={`text-xs whitespace-pre-wrap bg-white/50 p-3 rounded-xl border border-blue-100 max-h-32 overflow-y-auto ${formData.additionalRequirements ? 'text-blue-900' : muted}`}>
            {formData.additionalRequirements || 'Seleccionar'}
          </p>
        )}
      </div>
    </div>
